"""pytest configuration for the bpost test suite."""
import sys
...
def auto_enable_custom_integrations(enable_custom_integrations):
    """Make ``custom_components.bpost`` loadable from config-flow / setup tests."""
    yield
...
def reset_one_shot_warnings():
    ...
    Every one-shot warning in ``parcels.py`` is module-level bookkeeping, so
