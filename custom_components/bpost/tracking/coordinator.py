"""Coordinator for the bpost parcel tracker integration.
...
from .api import BpostApiClient, BpostApiError
from .parcels import (
...
class BpostCoordinator(DataUpdateCoordinator[list[dict]]):
    """Polls each tracked parcel and publishes the canonical parcel lists.

    This carrier has no account or parcel feed, so the tracked parcels are
    the barcodes the user entered (stored in the entry options), each paired
    with the hub's postal code default for the fetch. Each pair is fetched
    ...
    def _tracked(self) -> list[tuple[str, str]]:
        """Return the configured (barcode, postal_code) pairs.

        The postal code is the hub default (``entry.options[CONF_POSTAL_CODE]``),
        not a per-parcel value — see config_flow.py's hub-per-postcode model.
        """
        postal_code = self.config_entry.options.get(CONF_POSTAL_CODE)
        if not postal_code:
            return []
        return [
            (item[CONF_BARCODE], postal_code)
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        # cache stays bounded.
        tracked_keys = {parcel_key(barcode) for barcode, _ in tracked}
        self._raw_cache = {
        ...
        for (barcode, postal_code), result in zip(tracked_to_fetch, results):
            ...
            if isinstance(result, BaseException):
                if not isinstance(result, (BpostApiError, aiohttp.ClientError)):
                    raise result
                errors += 1
                if isinstance(result, BpostApiError) and result.status_code == 429:
                    saw_429 = True
            ...
            if result is None:
                # Unknown pair, or not scanned yet. Keep prior data if we
    ...
    def _fire_change_events(self, parcels: list[dict]) -> None:
        """Fire the incoming event set for this cycle's parcels."""
        fire_incoming_change_events(
            self.hass,
            parcels,
            self._known_state,
