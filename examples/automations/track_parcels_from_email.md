# Automatic parcel tracking from e-mail (IMAP → bpost)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the bpost tracking code, and register it with `bpost.track_parcel` — fully automatic, no extra custom component required.

bpost is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `bpost.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ bpost.track_parcel
                                     │
...
```
(?<![0-9A-Za-z])[A-Z]{2}[0-9]{8,11}[A-Z]{2}(?![0-9A-Za-z])
```
...
adjust the pattern**; the AI fallback will surface anything the regex
...
1. **Jinja eats backslashes in string literals.** A template stored as `regex_findall('\bCE…')` silently becomes a **backspace character** (`\b` is a string escape), so the regex never matches — no error anywhere. That's why the pattern is backslash-free: `(?<![0-9A-Za-z])` lookarounds instead of `\b`, `[0-9]` instead of `\d`. Copy that style if you adjust the pattern.
